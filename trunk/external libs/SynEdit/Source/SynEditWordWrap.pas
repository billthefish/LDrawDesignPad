...
$Id: SynEditWordWrap.pas,v 1.1 2004-03-01 22:17:18 billthefish Exp $
...
begin
  ...
  // Rewrap.
  if (RowCount - fLineOffsets[aIndex]) * aCount > RowCount * 5 then
  begin
    // This rudimentary optimization-wannabe tries to decide whether it's worth to
    // use ReWrapLine() since this may move the tail rowlengths too many times.
    WrapLines;
    Result := RowCount - aIndex +1;
  end
  else begin
    Result := 0;
    for cLine := aIndex to aIndex + aCount -1 do
...
begin
  ReallocMem( fLineOffsets, LineCount );
  fLineCapacity := LineCount;
  ReallocMem( fRowLengths, RowCount );
