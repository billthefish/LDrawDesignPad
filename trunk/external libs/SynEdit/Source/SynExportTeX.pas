...
$Id: SynExportTeX.pas,v 1.5 2004-03-01 22:17:01 billthefish Exp $
...
const
  ...
  NewCommand    = '\newcommand{\%s}[1]{%s%s%s%s#1%s}' + NewLine;
  SItalic       = '\textit{';
  SBold         = '\textbf{';
...
begin
  if not fCreateTeXFragment then begin
    if fPageStyleEmpty then
      ...
      PageStyle := '';
    Result := Format(TeXHeader + NewLine + NewLine, [Font.Size, fMargin, GetNewCommands, Title, PageStyle]);
  end;
